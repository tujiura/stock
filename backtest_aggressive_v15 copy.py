...
# 設定
HISTORY_FILE = "backtest_history_v15_fast.csv"
LOG_FILE = "ai_trade_memory_v15_aggressive.csv"
...
def calculate_metrics_v15(df, idx, market_regime_val):
    try:
        ...
        elif adx > ADX_MAX: regime = "Super Trend" 
        else: regime = "Weak"
...
class MemorySystem:
    def __init__(self, csv_path):
        ...
        self.feature_cols = ['adx', 'roc', 'vwap_dev', 'rsi'] 
        self.load_and_train()
    ...
    def get_similar_cases_text(self, metrics):
        ...
        try:
            vec_data = [metrics.get('adx',0), metrics.get('roc',0), metrics.get('vwap_dev',0)]
            vec_df = pd.DataFrame([vec_data], columns=['adx', 'roc', 'vwap_dev'])
            vec_norm = self.scaler.transform(vec_df)
...
def ai_market_commander_aggressive(model, chart_bytes, metrics):
    if model is None:
        return {"directive": "GO", "reason": "Default"}
    prompt = f"""
### Role
AI運用司令官 (Berserker Mode)
...
### Doctrine (V15)
1. **Attack First**: 市場がBearishでも、個別に吹き上げている銘柄はある。原則 **"GO"**。
2. **No Fear**: 200MAが下向いていようが関係ない。リバウンドを取りに行く。
3. **Wait is Death**: 待機(WAIT)は機会損失だ。

### Output
{{ "directive": "GO" or "WAIT", "reason": "理由" }}
"""
    try:
...
def main():
    print(f"=== V15 Backtest Aggressive (Fast Mode) ===")
    ...
    for date in dates:
        ...
        try:
            if date in nikkei.index:
                ...
                market_metrics = {'market_regime': m_row['Market_Regime'], 'sma200_slope': m_row.get('SMA200_Slope', 0.0)}
        ...
        # 2. 個別銘柄スキャン (APIを呼ばずに一次選考)
        current_prices = {}
        ai_candidates = [] # ★ 高速化の鍵: 候補リスト
        ...
        for ticker, df in market_data.items():
            ...
            if ticker not in portfolio.positions:
                ...
                ai_candidates.append({
                    'ticker': ticker, 'score': metrics['roc'],
                    'metrics': metrics, 'df': df, 'row': row, 'idx': idx
                })

        # 3. AI選抜試験 (上位5銘柄のみAPIコール)
        if ai_candidates:
            # ROCが高い順に並べ替え
            ai_candidates.sort(key=lambda x: x['score'], reverse=True)
            top_candidates = ai_candidates[:5] # 上位5つ
            ...
            for item in top_candidates:
                ...
                chart_bytes = create_chart_image(item['df'].iloc[:item['idx']+1], ticker)
                
                decision = ai_decision_maker_aggressive(model, chart_bytes, metrics, cbr_text, ticker, commander_instruction)
                
                if decision.get('action') == 'BUY':
                     portfolio.enter_position(date, ticker, item['row'], decision)
